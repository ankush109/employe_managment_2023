...
// exports.createEmployefe = async (req, res) => {
...
exports.createEmployee = async (req, res) => {
  ...
  try {
    ...
    bcrypt.hash(user.password, saltRounds, (err, hash) => {
      ...
      Promise.all([employee.save(), user.save()])
        .then(([employeeData, userData]) => {
          jwt.sign(
            ...
            (err, token) => {
              ...
              res
                .status(201)
                .json({ employee: employeeData, user: userData, token });
            }
          );
        })
        .catch((err) => {
          ...
          res
            .status(500)
            .json({ message: "Error adding Employee to database" });
        });
    });
  } catch (err) {
    console.log(err, "err");
